...
import frame.plugin.course.FaceService;

/**
 * 将此 DemoController 在YourJFinalConfig 中注册路由， 并设置好weixin开发者中心的 URL 与 token ，使 URL 指向该 DemoController 继承自父类 WeixinController 的 index 方法即可直接运行看效果，在此基础之上修改相关的方法即可进行实际项目开发
 */
@Coller(value = { "/wechat/msg" }, path = "wechat")
public class WxMsgCtrl extends MsgControllerAdapter {
	...
	static Log logger = Log.getLog(WxMsgCtrl.class);
	...
	protected void processInTextMsg(InTextMsg inTextMsg) {
		...
		if ("help".equalsIgnoreCase(msgContent) || "帮助".equals(msgContent)) {
			...
			msgContent = "请发一张清晰的照片！" + WeixinKit.emoji(0x1F4F7);
		...
		// 音乐消息测试
			...
			OutNewsMsg outMsg = new OutNewsMsg(inTextMsg);
			outMsg.addNews("JFinal 宝贝更新喽", "jfinal 宝贝更新喽，我们只看美女 ^_^", "https://mmbiz.qlogo.cn/mmbiz/KJoUl0sqZFRHa3VrmibqAXRfYPNdiamFnpPTOvXoxsFlRoOHbVibGhmHOEUQiboD3qXWszKuzWpibFxsVW1RmNB9hPw/0?wx_fmt=jpeg", "http://mp.weixin.qq.com/s?__biz=MzA4NjM4Mjk2Mw==&mid=211356950&idx=1&sn=6315a1a2848aa8cb0694bf1f4accfb07#rd");
			...
			ApiResult result = TemplateMsgApi.send(TemplateData.New()
					...
					// 模板id
					.setTemplate_id("tz1w-1fSN_Rzgj9_PjYuLU1EujvjPbtr1LL-_NtO6IM").setTopcolor("#743A3A").setUrl("http://img2.3lian.com/2014/f5/158/d/86.jpg")

					// 模板参数
					.add("first", "您好,Javen,欢迎使用模版消息!!\n", "#999").add("keyword1", "微信公众平台测试", "#999").add("keyword2", "39.8元", "#999").add("keyword3", "yyyy年MM月dd日 HH时mm分ss秒", "#999").add("remark", "\n您的订单已提交，我们将尽快发货，祝生活愉快! 点击可以查看详细信息。", "#999").build());
